...
#set classes
for(i in seq(1:9)){
  ldf[[1]]$latitude <- as.numeric(as.character(ldf[[i]]$latitude))
...
ggplot(ldf[[1]], aes(x = longitude, y = latitude, color = density))+
  geom_point(size = 3)+
  scale_color_gradient2(low = scales::muted("blue"), mid = "white", high = scales::muted("red"), midpoint = mean(ldf[[1]]$density)) +
